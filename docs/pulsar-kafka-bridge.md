...
2.  **吞吐与延迟的矛盾**：`StreamLoad` 是一个同步的、事务性的导入方式。
      ...
      * **为了低延迟**：可能会选择来一条数据就推送一次，但这会产生大量小的 `StreamLoad` 任务，给 StarRocks FE 带来巨大压力，整体吞吐量极低。
      * **为了高吞吐**：需要自己实现复杂的攒批逻辑（比如每秒或每10MB数据提交一次），这增加了代码复杂性和内存消耗，并牺牲了数据的实时性。

3.  **无原生反压机制**：如果 StarRocks 后端因为 Compaction、查询压力等原因变慢，`StreamLoad` 的 HTTP 请求会超时或失败。自定义服务需要处理这些异常，但缺乏一个优雅的反压（Backpressure）机制。通常结果是服务不断重试，消耗大量资源，甚至内存溢出而崩溃。
